...
export const WarrantyStatus = { PENDING: 'PENDING', REVIEWING: 'REVIEWING', APPROVED: 'APPROVED', REJECTED: 'REJECTED', REPLACED: 'REPLACED', REFUNDED: 'REFUNDED' } as const;
export interface WarrantyRequest {
  requestCode: string; userId: Types.ObjectId; orderId: Types.ObjectId; inventoryItemId: Types.ObjectId;
  reason: string; evidenceUrls: string[]; status: typeof WarrantyStatus[keyof typeof WarrantyStatus];
...
export const WarrantyRequestSchema = new Schema<WarrantyRequest>({
  ...
  inventoryItemId: objectId('InventoryItem', true), reason: { type: String, required: true, maxlength: 5000 },
